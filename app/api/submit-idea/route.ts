  ...
  try {
    ...
    if (!name || !contact || !description) {
      return NextResponse.json(
        { success: false, message: 'Имя, контакт и описание идеи обязательны для заполнения' },
        { status: 400 }
      );
    }
    ...
    if (!emailRegex.test(contact) && !phoneRegex.test(contact.replace(/\s/g, ''))) {
      return NextResponse.json(
        { success: false, message: 'Введите корректный номер телефона или email' },
        { status: 400 }
      );
    }
...
⏰ Время: ${new Date().toLocaleTimeString('ru-RU')}`;
    ...
    return NextResponse.json({
      success: true,
      message: 'Ваша идея успешно отправлена! Мы свяжемся с вами в ближайшее время для обсуждения проекта.',
      thankYou: 'Спасибо за доверие к нашей лаборатории! Ваша идея уже в надежных руках наших экспертов.'
    });

  } catch (error) {
    console.error('Error submitting idea:', error);
    return NextResponse.json(
      { success: false, message: 'Произошла ошибка при отправке идеи. Пожалуйста, попробуйте еще раз.' },
      { status: 500 }
    );
  }
}